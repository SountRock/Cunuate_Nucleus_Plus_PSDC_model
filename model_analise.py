...
import json
import numpy as np
from getStruct import getNormalStruct_5_2
import os
import glob
from analyzers.plotsPatterns import psi_analysis_top, globalize_and_wct_plot_from_peaks, load_spike_data_from_checkpoint, transformDictToMatrix, filterAndRenumberDict, align_by_first_spike, save_raster_data_to_json, load_pop_spikes_from_json, plot_ISI_analysis, save_phase_histogram_data_to_json, plot_phase_histogram_from_json, save_phase_difference_data_to_json, plot_phase_difference_histogram_from_json, plot_kde_from_spike_groups, plot_spike_spectrogram2
...
def megaTargetFuncOnlySyn(savePath, 
    ...
    ItoE_weight_SA = abs(ItoE_weight_SA)
    InterPSDC_weight_SA = abs(InterPSDC_weight_SA) 
    PANtoE_weight_SA = abs(PANtoE_weight_SA) 
    PANtoI_weight_SA = abs(PANtoI_weight_SA) 

    ItoE_weight_RA = abs(ItoE_weight_RA) 
    InterPSDC_weight_RA = abs(InterPSDC_weight_RA) 
    PANtoE_weight_RA = abs(PANtoE_weight_RA) 
    PANtoI_weight_RA = abs(PANtoI_weight_RA) 

    ItoE_w_min_SA = abs(ItoE_w_min_SA) 
    InterPSDC_w_min_SA = abs(InterPSDC_w_min_SA) 
    PANtoE_w_min_SA = abs(PANtoE_w_min_SA) 
    PANtoI_w_min_SA= abs(PANtoI_w_min_SA) 

    ItoE_w_min_RA = abs(ItoE_w_min_RA) 
    InterPSDC_w_min_RA = abs(InterPSDC_w_min_RA) 
    PANtoE_w_min_RA = abs(PANtoE_w_min_RA) 
    PANtoI_w_min_RA= abs(PANtoI_w_min_RA) 
    ...
    for inputsFile in inputsFiles:
        ...
        for key, value in dictTimes.items():
            ...
            simConfig.duration = total_time + 500 #A small margin for Wavelet-coherence       
            simConfig.dt = 0.025 
...
#For vibro model 
with open("optimisation/best_init_params/vibro_model/paramsRA_x.json", "r", encoding="utf-8") as f:
    init_params_RA = json.load(f)
with open("optimisation/best_init_params/vibro_model/paramsSA_x.json", "r", encoding="utf-8") as f:
    init_params_SA = json.load(f)

path = f'hard_pattern_vibro'
os.makedirs(path, exist_ok=True)
params = {**init_params_RA, **init_params_SA}

'''
#For pressure model 
path = f'hard_pattern_pressure'
os.makedirs(path, exist_ok=True)

with open("optimisation/best_init_params/pressure_model/params.json", "r", encoding="utf-8") as f: 
    params = json.load(f)
'''
